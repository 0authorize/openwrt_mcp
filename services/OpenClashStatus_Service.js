...
if [ -f /etc/config/openclash ]; then
    ...
    uci show openclash | grep -E "config|enable|config_path" || true;
...
if [ -n "$CONFIG" ] && [ -f "$CONFIG" ]; then
    ...
    grep -E "^mode:|^mixed-port:|^redir-port:|^tproxy-port:|^tun:" $CONFIG || true;
...
if [ -n "$CONFIG" ] && [ -f "$CONFIG" ]; then
    ...
    grep -A5 "fake-ip" "$CONFIG" | head -10 || true;
...
if [ -f /tmp/openclash.log ]; then
    ...
    tail -30 /tmp/openclash.log | grep -iE "error|fail|warning" || true;
